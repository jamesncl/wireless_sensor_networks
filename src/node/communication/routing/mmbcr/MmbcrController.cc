#include "MmbcrController.h"

// Register the module in Omnet++
Define_Module(MmbcrController);

const char * MmbcrController::OUTPUT_MMBCR_DROPPED_AFTER_MAX_RETRIES = "Mmbcr dropped packet after max retries";
const char * MmbcrController::OUTPUT_MMBCR_DROPPED_OUT_OF_ENERGY = "Mmbcr dropped packet from buffer out of energy";
const char * MmbcrController::OUTPUT_MMBCR_SENDING_RETRY = "Mmbcr retrying send";
const char * MmbcrController::OUTPUT_MMBCR_FORWARDING = "Mmbcr received packet for forwarding";
const char * MmbcrController::OUTPUT_MMBCR_HOP_COUNT = "Mmbcr hop count";

void MmbcrController::startup()
{
	...
	if(!hasStartedUpOnce)
	{
		...
		declareOutput(OUTPUT_MMBCR_DROPPED_AFTER_MAX_RETRIES);
		declareOutput(OUTPUT_MMBCR_DROPPED_OUT_OF_ENERGY);
		declareOutput(OUTPUT_MMBCR_SENDING_RETRY);
		declareOutput(OUTPUT_MMBCR_FORWARDING);
		declareHistogram(OUTPUT_MMBCR_HOP_COUNT, 1, 10, 10);
	...
	{
		setTimer(MMBCR_CONTROLLER_TIMER_DELAY_ROUTE_DISCOVERY_PROPAGATION, delayBeforeRouteDiscoveryPropagation);
	}
}

void MmbcrController::initiateRouteDiscoveryAndPropagation()
{
	...
	MmbcrBeaconSenderControlMessage *resetTrickleMsg = new MmbcrBeaconSenderControlMessage("Reset trickle and pull message", BEACON_SENDER_CONTROL_COMMAND);
	resetTrickleMsg->setBeaconSenderControlMessageKind(MMBCR_BEACON_SENDER_RESET_TRICKLE_AND_PULL);
	send(resetTrickleMsg, "toBeaconSender");
}

void MmbcrController::handleOutOfEnergy(cMessage *outOfEnergyMsg)
{
	...
	while (!TXBuffer.empty()) {
		...
		TXBuffer.pop();
		cancelAndDelete(pkt);
		collectOutput(OUTPUT_MMBCR_DROPPED_OUT_OF_ENERGY);
	}
...
void MmbcrController::fromApplicationLayer(cPacket *pkt, const char *destination)
{
	...
	if(atoi(destination) != sinkNodeId || isSink) {
		opp_error("MMBCR expects apps to specify destination (%s) as sink node, and sink nodes shouldn't send app packets", destination);
	}

	trace() << "Received packet from application layer. Current parent is " << currentParentNodeId; 

	// Create the packet
	MmbcrPacket *networkPacket = new MmbcrPacket("MMBCR routing data packet", NETWORK_LAYER_PACKET);
	...
	networkPacket->setRoutingPacketKind(MMBCR_PACKET_TYPE_DATA);
...
void MmbcrController::sendPackets()
{
	...
	if(TXBuffer.size() > 0)
	{
		if(implementRetries)
		{
			if(currentPacketSendingAttempts < maxPacketSendRetries)
			{
				...
				//plotTrace() << "#ROU_SEND";
				MmbcrPacket *networkPacket = check_and_cast<MmbcrPacket*>(TXBuffer.front()->dup());
			...
			else // Max number of retries attempted - drop the packet
			{
				...
				currentPacketSendingAttempts = 0;
				// Collect stats
				collectOutput(OUTPUT_MMBCR_DROPPED_AFTER_MAX_RETRIES);
		...
		else
		{
			...
			//plotTrace() << "#ROU_SEND";
			MmbcrPacket *networkPacket = check_and_cast<MmbcrPacket*>(TXBuffer.front());
			TXBuffer.pop();
			trace() << "Sending packet (attempt number " << currentPacketSendingAttempts << "): origin " << networkPacket->getOrigin() << ", " <<
					"sequenceNo " << networkPacket->getSequenceNumber() << ", " <<
					"hop count " <<networkPacket->getHopCount() << " " <<
...
void MmbcrController::fromMacLayer(cPacket *pkt, int srcMacAddress, double rssi, double lqi)
{
	MmbcrPacket *ctpPkt = check_and_cast<MmbcrPacket*>(pkt);

	if(!ctpPkt) {
		opp_error("Expected MMBCR routing packet");
	}
	...
	switch(ctpPkt->getRoutingPacketKind()){
		...
		case MMBCR_PACKET_TYPE_BEACON: {
			...
			trace() << "MMBCR - Received beacon " << ctpPkt->getSequenceNumber() << " from node " << ctpPkt->getNetMacInfoExchange().lastHop;
			//plotTrace() << "#ROU_REC_BEACON";

			// Send to the TableManager so we can update battery capacities to sink
			send(ctpPkt->dup(), "toTableManager");
			...
			if(ctpPkt->getPullFlag())
			{
				...
				MmbcrBeaconSenderControlMessage *resetTrickleMsg = new MmbcrBeaconSenderControlMessage("Reset trickle message", BEACON_SENDER_CONTROL_COMMAND);
				resetTrickleMsg->setBeaconSenderControlMessageKind(MMBCR_BEACON_SENDER_RESET_TRICKLE);
				send(resetTrickleMsg, "toBeaconSender");
			}
		...
		case MMBCR_PACKET_TYPE_DATA: {
			...
			if(ctpPkt->getNetMacInfoExchange().nextHop == self)
			{
				...
				if(isSink)
				{
					...
					collectHistogram(OUTPUT_MMBCR_HOP_COUNT, ctpPkt->getHopCount());			
				}
		...
		default: {
			opp_error("Unexpected MMBCR routing packet type");
		}
	}
}

void MmbcrController::forwardPacket(MmbcrPacket *pkt)
{
	trace() << "Forwarding packet number " << pkt->getSequenceNumber() << " from " << pkt->getNetMacInfoExchange().lastHop <<
	 	", origin " << pkt->getOrigin();
	collectOutput(OUTPUT_MMBCR_FORWARDING);
...
void MmbcrController::repairLoop()
{
	...
	setTimer(MMBCR_CONTROLLER_TIMER_LOOP_REPAIR, repairLoopWaitTime);
}


bool MmbcrController::isDuplicate(MmbcrPacket *pkt)
{
	...
	mmbcrPacket_type type = (mmbcrPacket_type) pkt->getRoutingPacketKind();
	...
	for (iter = duplicateDetectionBuffer[origin].begin(); iter != duplicateDetectionBuffer[origin].end(); iter++) 
	{
		...
		if((*iter).type == (mmbcrPacket_type) pkt->getRoutingPacketKind()
	...
	entry.type =  (mmbcrPacket_type) pkt->getRoutingPacketKind();
...
void MmbcrController::clearDuplicateBuffer()
{
...
void MmbcrController::handleMacControlMessage(cMessage *msg)
{
...
void MmbcrController::handleRadioControlMessage(cMessage *msg)
{
...
void MmbcrController::handleNetworkControlCommand(cMessage *msg) 
{ 
	switch(msg->getKind()) {
			...
			switch(controlMsg->getRoutingControlMessageKind()) {
				...
				case ROUTING_MSG_MAC_SENDING_FAILED_NO_ACK: {
					...
					if(implementRetries)
					{
						...
						collectOutput(OUTPUT_MMBCR_SENDING_RETRY);
					}
				...
				case ROUTING_MSG_SINK_NODE_UPDATE: {
					...
					// Pass a copy of this update on to the table manager, so that the table manager
					// can record this node's MH-ETX as zero if this is the sink node
					send(controlMsg->dup(), "toTableManager");
					// Also pass to BeaconSender - the beacon sender also needs to know if this is sink,
					// so it can send MH-ETX of zero and battery capacity as N/A in beacons
					send(controlMsg->dup(), "toBeaconSender");
		...
		{
			MmbcrControlMessage *controlMsg = check_and_cast<MmbcrControlMessage*>(msg);

			switch(controlMsg->getMmbcrControlMessageKind()) 
			{
				// The table manager is updating the controller of the latest chosen parent node ID and multihop ETX
				case MMBCR_MSG_UPDATE_ROUTE_INFO: {
				...
				default: {
					opp_error("Unknown MMBCR network control message type");
				}
			}
			break;
		}
...
void MmbcrController::timerFiredCallback(int index)
{
	switch(index)
	{
		case MMBCR_CONTROLLER_TIMER_LOOP_REPAIR: {
		...
		case MMBCR_CONTROLLER_TIMER_DELAY_ROUTE_DISCOVERY_PROPAGATION: {
			initiateRouteDiscoveryAndPropagation();
			break;
		}
...
void MmbcrController::finishSpecific()
{
	clearDuplicateBuffer();
}